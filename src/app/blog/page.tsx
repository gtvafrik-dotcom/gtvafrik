...
export default async function BlogPage() {
    ...
    const getArticles = (cat: string, limit: number) => {
        const filtered = latestArticles.filter((a: any) => a.categories.some((c: any) => c.category.name.toLowerCase() === cat.toLowerCase()));
        return filtered.length > 0 ? filtered.slice(0, limit) : latestArticles.slice(0, limit);
    };
        ...
        <div className="min-h-screen bg-white font-gudlak overflow-x-hidden selection:bg-brand-yellow selection:text-brand-dark-navy">
            ...
            <section className="bg-brand-dark-navy text-white px-6 md:px-16 pt-10 pb-16 md:pb-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                    <Link href={`/blog/${headlines[0]?.slug || '#'}`} className="md:col-span-8 aspect-[4/5] md:aspect-auto md:h-[450px] bg-brand-vibrant-blue rounded-xl shadow-2xl relative block overflow-hidden group">
                        {headlines[0]?.thumbnail && <img src={headlines[0].thumbnail} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />}
                    ...
                    <div className="hidden md:flex md:col-span-4 flex-col gap-5">
                        {headlines.slice(1, 3).map((article: any, idx: number) => (
                            <Link href={`/blog/${article?.slug || '#'}`} key={idx} className="flex-1 bg-brand-vibrant-blue rounded-xl relative overflow-hidden group">
                                {article?.thumbnail && <img src={article.thumbnail} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />}
            ...
            <div className="bg-[#D42027] text-white py-3.5 px-6 md:px-16 flex items-center overflow-hidden">
                ...
                <div className="flex gap-12 whitespace-nowrap text-[11px] font-prompt font-medium tracking-tight overflow-hidden">
                    <span className="truncate">{latestArticles[0]?.title || 'Welcome to GTV Afrik Blog'}</span>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-16">
                    <Link href={`/blog/${breaking[0]?.slug || '#'}`} className="md:col-span-4 bg-white border border-gray-100 overflow-hidden rounded-2xl shadow-sm flex flex-col h-full group">
                        <div className="aspect-[4/3] bg-brand-vibrant-blue relative p-5 overflow-hidden">
                            {breaking[0]?.thumbnail && <img src={breaking[0].thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                        ...
                        <div className="p-8 md:p-10 flex-1 flex flex-col justify-center">
                            <h3 className="text-[20px] md:text-[22px] font-bold text-brand-dark-navy mb-5 leading-tight">{breaking[0]?.title || 'Rethinking Travel in a Warming World'}</h3>
                            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest font-prompt">{breaking[0]?.categories?.[0]?.category.name || 'News'} • Recent</p>
                        </div>
                    </Link>

                    <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-x-10 lg:gap-x-14 gap-y-10">
                        {(breaking.length > 1 ? breaking.slice(1, 7) : Array(6).fill(null)).map((article: any, i: number) => (
                            <Link href={`/blog/${article?.slug || '#'}`} key={i} className="flex gap-3 group cursor-pointer border-b border-gray-100 pb-8">
                                <div className="w-16 h-16 md:w-24 md:h-24 bg-gray-200 rounded-xl shrink-0 group-hover:scale-95 transition-transform overflow-hidden relative">
                                    {article?.thumbnail && <img src={article.thumbnail} className="absolute inset-0 w-full h-full object-cover" alt="" />}
                                </div>
                                <div className="flex flex-col justify-center">
                                    <span className="text-brand-vibrant-blue font-bold text-[8.5px] uppercase tracking-widest mb-2.5 font-prompt">{article?.categories?.[0]?.category.name || 'Media'}</span>
                                    <h4 className="text-[13px] md:text-[14.5px] font-bold text-brand-dark-navy leading-tight">{article?.title || 'World Autism Day: Specialised school shortage hinders education'}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <section className="bg-brand-dark-navy px-6 md:px-16 py-20 md:py-28 text-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {(gtv.length > 0 ? gtv : Array(3).fill(null)).map((article: any, i: number) => (
                        <Link href={`/blog/${article?.slug || '#'}`} key={i} className="bg-white rounded-2xl overflow-hidden shadow-2xl group cursor-pointer block">
                            <div className="aspect-video bg-brand-vibrant-blue relative overflow-hidden">
                                {article?.thumbnail && <img src={article.thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                                <PlayButton />
                            </div>
                            <div className="p-6 md:p-8 text-brand-dark-navy">
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3 font-prompt">{article?.categories?.[0]?.category.name || 'Politics'} · Videos</p>
                                <h3 className="text-[14px] md:text-[15px] font-bold leading-snug mb-5 md:mb-6 line-clamp-2">{article?.title || "Chief Edward David Onoja: Reflecting on Leadership, Tinubu's Vision..."}</h3>
                                <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest font-prompt">Recent</p>
                            </div>
                        </Link>
                    ))}
                </div>
            ...
            <section className="px-6 md:px-16 py-16 md:py-28 bg-[#F4F6FF]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {(politics.length > 0 ? politics : Array(3).fill(null)).map((article: any, i: number) => (
                        <Link href={`/blog/${article?.slug || '#'}`} key={i} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-shadow block">
                            ...
                            <div className="p-6 md:p-8">
                                ...
                                <h3 className="text-[15px] font-bold text-brand-dark-navy leading-snug mb-5 line-clamp-2">{article?.title || 'PDP Convention Shows True Power Is Structure, Not Social Media Noise'}</h3>
                                <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest font-prompt">Recent</p>
                            </div>
                        </Link>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-16">
                    <Link href={`/blog/${international[0]?.slug || '#'}`} className="md:col-span-4 bg-white border border-gray-100 overflow-hidden rounded-2xl shadow-sm flex flex-col h-full group">
                        <div className="aspect-[4/3] bg-brand-vibrant-blue relative overflow-hidden">
                            {international[0]?.thumbnail && <img src={international[0].thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                        </div>
                        <div className="p-8 md:p-10 flex-1 flex flex-col justify-center">
                            <p className="text-[10px] font-bold text-brand-dark-navy uppercase tracking-widest mb-4 font-prompt">International</p>
                            <h3 className="text-[20px] md:text-[22px] font-bold text-brand-dark-navy mb-5 leading-tight">{international[0]?.title || 'Uganda Airlines Emergency Return. What Aviation Experts Say'}</h3>
                            <p className="text-gray-500 font-prompt text-[13px] mb-8 leading-relaxed font-light hidden md:block line-clamp-3">{international[0]?.excerpt || "Specialists weigh in on bird strike risks and airline safety protocols across East Africa's growing aviation sector."}</p>
                            <p className="text-gray-400 text-[9px] font-bold uppercase tracking-widest font-prompt">Recent</p>
                        </div>
                    </Link>
                    <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-x-10 lg:gap-x-14 gap-y-10">
                        {(international.length > 1 ? international.slice(1, 7) : Array(6).fill(null)).map((article: any, i: number) => (
                            <Link href={`/blog/${article?.slug || '#'}`} key={i} className="flex gap-3 group cursor-pointer border-b border-gray-100 pb-8">
                                ...
                                <div className="flex flex-col justify-center">
                                    ...
                                    <h4 className="text-[13px] md:text-[14.5px] font-bold text-brand-dark-navy leading-tight">{article?.title || 'AI Health Messaging in Kenya and Nigeria: A Surprising Verdict'}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <div className="px-6 md:px-16 py-10 md:py-28 bg-[#F4F6FF] flex flex-col md:grid md:grid-cols-2 gap-16 md:gap-20">
                <section>
                    ...
                    <Link href={`/blog/${entertainment[0]?.slug || '#'}`} className="bg-white rounded-2xl overflow-hidden mb-6 md:mb-10 shadow-sm block group">
                        <div className="aspect-video bg-brand-vibrant-blue relative overflow-hidden">
                            {entertainment[0]?.thumbnail && <img src={entertainment[0].thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                            <PlayButton />
                        </div>
                        <div className="p-6 md:p-8">
                            <p className="text-[9px] font-bold text-brand-dark-navy/60 uppercase tracking-widest mb-2 font-prompt">Music · Culture</p>
                            <h3 className="text-[15px] md:text-[16px] font-bold text-brand-dark-navy leading-snug mb-3">{entertainment[0]?.title || 'Tributes Flow for Late Musician Yullander'}</h3>
                            <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest font-prompt">Recent</p>
                        </div>
                    </Link>
                    <div className="flex flex-col gap-6 md:gap-8">
                        {(entertainment.length > 1 ? entertainment.slice(1, 3) : Array(2).fill(null)).map((article: any, i: number) => (
                            <Link href={`/blog/${article?.slug || '#'}`} key={i} className="flex gap-5 group cursor-pointer">
                                ...
                                <div className="flex flex-col justify-center">
                                    <span className="text-brand-vibrant-blue font-bold text-[8.5px] uppercase tracking-widest mb-1 font-prompt">Culture</span>
                                    <h4 className="text-[13px] font-bold text-brand-dark-navy leading-snug">{article?.title || 'Tributes Flow for Late Musician Yullander'}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
                ...
                <section>
                    ...
                    <Link href={`/blog/${health[0]?.slug || '#'}`} className="bg-white rounded-2xl overflow-hidden mb-6 md:mb-10 shadow-sm block group">
                        <div className="aspect-video bg-brand-vibrant-blue relative overflow-hidden">
                            {health[0]?.thumbnail && <img src={health[0].thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                            <PlayButton />
                        </div>
                        <div className="p-6 md:p-8">
                            <p className="text-[9px] font-bold text-brand-dark-navy/60 uppercase tracking-widest mb-2 font-prompt">Health</p>
                            <h3 className="text-[15px] md:text-[16px] font-bold text-brand-dark-navy leading-snug mb-3">{health[0]?.title || 'World Autism Day: Specialised School Shortage Hinders Education'}</h3>
                            <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest font-prompt">Recent</p>
                        </div>
                    </Link>
                    <div className="flex flex-col gap-6 md:gap-8">
                        {(health.length > 1 ? health.slice(1, 3) : Array(2).fill(null)).map((article: any, i: number) => (
                            <Link href={`/blog/${article?.slug || '#'}`} key={i} className="flex gap-5 group cursor-pointer">
                                ...
                                <div className="flex flex-col justify-center">
                                    ...
                                    <h4 className="text-[13px] font-bold text-brand-dark-navy leading-snug">{article?.title || 'AI Health Messaging in Kenya and Nigeria: A Surprising Verdict'}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <section className="bg-brand-dark-navy px-6 md:px-16 py-20 md:py-28 text-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {(sports.length > 0 ? sports : [
                        ...
                        <Link href={`/blog/${item?.slug || '#'}`} key={i} className="bg-white rounded-2xl overflow-hidden shadow-lg group cursor-pointer block">
                            <div className="h-48 bg-brand-vibrant-blue group-hover:opacity-90 transition-opacity relative overflow-hidden">
                                {item?.thumbnail && <img src={item.thumbnail} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform" alt="" />}
                            </div>
                            <div className="p-6 md:p-8 text-brand-dark-navy">
                                <p className="text-[9px] font-bold text-brand-dark-navy/60 uppercase tracking-widest mb-3 font-prompt">{item?.categories?.[0]?.category.name || item.cat || 'Sports'}</p>
                                <h3 className="text-[15px] font-bold leading-tight mb-4 line-clamp-2">{item?.title || item.title}</h3>
                                <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest font-prompt">Recent</p>
                            </div>
                        </Link>
                    ))}
